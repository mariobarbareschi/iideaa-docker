...
extern "C" double BELLERO_Reward()
{
	...
	int stride4_norm = max(stride4, 5);
  ...
  double skipped_loops = (stride1_norm * stride2_norm) + (stride3_norm * stride4_norm) + (stride5_norm * stride6_norm);
	reward = skipped_loops / total_loops;

	printf("Strides: [%d, %d, %d, %d, %d, %d]\n", stride1_norm, stride2_norm, stride3_norm, stride4_norm, stride5_norm, stride6_norm);
	printf("Reward: %lf\n", reward);
