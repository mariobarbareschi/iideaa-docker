...
)
{
  ...
  for (unsigned int channel = 0; channel < CONV2_NB_CHANNELS; ++channel)
  {
    ...
    for (unsigned int sy = syMin; sy < syMax; ++sy)
      for (unsigned int sx = sxMin; sx < sxMax; ++sx)
      {
        SUM_T prod = (SUM_T) (evoapproxlib::evoapprox_t((int16_t)(*weights[output][channel])[sy][sx], evoApprox8u_component_1) * evoapproxlib::evoapprox_t((int16_t)((UDATA_T) inputs[channel][iy + sy][ix + sx]), evoApprox8u_component_1));
        weightedSum += prod;
      }
  }
  return weightedSum;
...
)
{
  ...
  for (unsigned int channel = 0; channel < CONV3_NB_CHANNELS; ++channel)
  {
    ...
    for (unsigned int sy = syMin; sy < syMax; ++sy)
      for (unsigned int sx = sxMin; sx < sxMax; ++sx)
      {
        SUM_T prod = (SUM_T) (evoapproxlib::evoapprox_t((int16_t)(*weights[output][channel])[sy][sx], evoApprox8u_component_2) * evoapproxlib::evoapprox_t((int16_t)((UDATA_T) inputs[channel][iy + sy][ix + sx]), evoApprox8u_component_2));
        weightedSum += prod;
      }
  }
  return weightedSum;
  ...
  for (unsigned int output = 0; output < FC1_NB_OUTPUTS; ++output)
  {
    ...
    for (unsigned int channel = 0; channel < CONV3_NB_OUTPUTS; ++channel)
      for (unsigned int iy = 0; iy < CONV3_OUTPUTS_HEIGHT; ++iy)
        ...
        {
          SUM_T prod = (SUM_T) (evoapproxlib::evoapprox_t((int16_t)weights[output][c++], evoApprox8u_component_3) * evoapproxlib::evoapprox_t((int16_t)((UDATA_T) inputs[channel][iy][ix]), evoApprox8u_component_3));
          weightedSum += prod;
        }
  ...
  const WDATA_T (&weights)[OUTPUTS_SIZE * NB_OUTPUTS][FC2_NB_CHANNELS])
{
  for (unsigned int output = 0; output < OUTPUTS_SIZE * NB_OUTPUTS; ++output)
  {
    ...
    {
      SUM_T prod = (SUM_T) (evoapproxlib::evoapprox_t((int16_t)weights[output][channel], evoApprox8u_component_4) * evoapproxlib::evoapprox_t((int16_t)((UDATA_T) inputs[channel]), evoApprox8u_component_4));
      weightedSum += prod;
    }
